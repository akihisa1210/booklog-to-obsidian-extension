 ...
 * Throws if the same item_id appears in multiple files: syncing would update
 * only one of them arbitrarily, so the duplicates must be resolved first.
 */
export async function buildIdBookIndex(api, booksDir) {
    ...
    if (match) {
      ...
      if (index[itemId]) {
        if (!duplicates[itemId]) {
          duplicates[itemId] = [index[itemId]];
        }
        duplicates[itemId].push(filePath);
      } else {
        index[itemId] = filePath;
      }
    }
    readCount++;
  }
  ...
  if (dupIds.length > 0) {
    for (const id of dupIds) {
      console.error(`duplicate item_id ${id}:`, duplicates[id]);
    }
    const example = duplicates[dupIds[0]]
      .map((p) => p.split("/").pop())
      .join(" と ");
    throw new Error(
      `同じ item_id を持つファイルが${dupIds.length}組あるため、同期を中断しました（例: ${example}）。` +
        "残す方のファイルにメモを写してからもう一方を削除し、再実行してください。" +
        "全ペアは Service Worker のコンソールに出力しています。",
    );
  }
