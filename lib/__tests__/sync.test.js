...
describe("serializeFrontmatter", () => {
  it("serializes Book to YAML frontmatter", () => {
    const book = createBook();
...
`,
  ...
  it("quotes title containing colon", () => {
    const book = createBook({ title: "タイトル: サブタイトル" });
    const yaml = serializeFrontmatter(book);
    expect(yaml).toContain("title: 'タイトル: サブタイトル'");
  });

  it("escapes single quotes in values", () => {
    const book = createBook({ author: "author's note" });
    const yaml = serializeFrontmatter(book);
    expect(yaml).toContain("author: 'author''s note'");
  });
});
...
describe("buildIdBookIndex", () => {
  ...
  it("throws when multiple files share the same item_id", async () => {
    const api = createMockApi({
      "Books/『旧形式』.md": "---\nitem_id: 1000000000\n---",
      "Books/作者『新形式』（出版社、2020）.md": "---\nitem_id: 1000000000\n---",
      "Books/Other.md": "---\nitem_id: 2000000000\n---",
    });

    await expect(buildIdBookIndex(api, "Books")).rejects.toThrow(
      /同じ item_id を持つファイルが1組/,
    );
  });
  ...
  it("throws when listFiles fails", async () => {
    ...
    await expect(buildIdBookIndex(api, "Books")).rejects.toThrow("Not found");
  });

  it("throws when readFile fails", async () => {
    const api = {
      listFiles: vi.fn().mockResolvedValue(["Book1.md"]),
      readFile: vi.fn().mockRejectedValue(new Error("Read error")),
      writeFile: vi.fn(),
    };

    await expect(buildIdBookIndex(api, "Books")).rejects.toThrow("Read error");
  });
});
...
面白かった`;
  ...
  it("throws when existing file has no frontmatter", async () => {
    const api = createMockApi({
      "Books/Broken.md": "# No frontmatter",
    });
    const book = createBook();

    await expect(
      saveBook(api, "Books", book, "Books/Broken.md"),
    ).rejects.toThrow("Failed to parse frontmatter in existing file: Books/Broken.md");
  });
